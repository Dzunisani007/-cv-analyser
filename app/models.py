...
class Resume(Base):
    ...
    updated_at = mapped_column(sa.DateTime(timezone=True), server_default=sa.func.now())
...
class CVAnalysis(Base):
    ...
    resume_id: Mapped[uuid.UUID | None] = mapped_column(
        sa.UUID(as_uuid=True), ForeignKey("cv_resumes.id", ondelete="CASCADE"), nullable=True
    )
...
class ResumeScore(Base):
    ...
    resume_id: Mapped[uuid.UUID | None] = mapped_column(sa.UUID(as_uuid=True), nullable=True)
    overall_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    component_scores = mapped_column(sa.JSON, nullable=True)
