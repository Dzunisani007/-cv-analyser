"""Initialize new analyser database tables

Revision ID: 20260321_new_analyser
Revises: 
...
revision = '20260321_new_analyser'
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table('cv_analyses',
        sa.Column('id', sa.UUID(as_uuid=True), nullable=False, primary_key=True, default=sa.text('uuid_generate_v4()')),
        sa.Column('record_id', sa.UUID(as_uuid=True), sa.ForeignKey('cv_records.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table('cv_resume_skills',
        ...
        sa.Column('resume_id', sa.UUID(as_uuid=True), sa.ForeignKey('cv_records.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table('cv_workflow_audit_logs',
        sa.Column('id', sa.BigInteger(), nullable=False, primary_key=True, autoincrement=True),
        sa.Column('analysis_id', sa.UUID(as_uuid=True), sa.ForeignKey('cv_analyses.id', ondelete='CASCADE'), nullable=False),
